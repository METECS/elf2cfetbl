...
void set_st_size(union Elf_Sym *Symbol, uint64_t new_value)
{
    if (TargetWordsizeIs32Bit)
    {
        ...
        if (Symbol->Sym32.st_size != new_value) {
            printf("ERROR: Sym32.st_size can not hold %" PRIu64 "\n", new_value);
        }
    }
    else
    {
    ...
    if ((SectionHeaderStringTableDataOffset != 0) && (get_sh_name(SectionHeader) != 0))
    {
        ...
        switch (get_sh_type(SectionHeader))
        {
            ...
            case SHT_SYMTAB:
                ...
                sprintf(VerboseStr, "SHT_SYMTAB (2) - # Symbols = %" PRIu64 "", NumSymbols);
                break;
...
int32 GetSymbol(int32 SymbolIndex, union Elf_Sym *Symbol)
{
    ...
    {
        printf("Error: SeekOffset may not be %" PRIu64 "\n", calculated_offset);
        Status = FAILED;
    }
    else
    {
...
void PrintSymbol64(union Elf_Sym *Symbol)
{
    if (Verbose) printf("   st_value = 0x%" PRIx64 "\n", Symbol->Sym64.st_value);
    if (Verbose) printf("   st_size  = 0x%08" PRIx64 "\n", Symbol->Sym64.st_size);
...
{
    if (Verbose) printf("   e_version = %d\n", get_e_version(&ElfHeader));
    if (Verbose) printf("   e_entry = 0x%" PRIx64 "\n", ElfHeader.Ehdr64.e_entry);
    if (Verbose) printf("   e_phoff = 0x%08" PRIx64 " (%" PRIu64 ")\n", ElfHeader.Ehdr64.e_phoff, ElfHeader.Ehdr64.e_phoff);
    if (Verbose) printf("   e_shoff = 0x%08" PRIx64 " (%" PRIu64 ")\n", ElfHeader.Ehdr64.e_shoff, ElfHeader.Ehdr64.e_shoff);
...
int32 GetTblDefInfo(void)
{
    ...
    {
        printf("Error! '%s' is not properly defined in '%s'.  Size of object is incorrect (%" PRId64 ").\n",
...
int32 LocateAndReadUserObject(void)
{
    ...
    if (Verbose) printf("\nTrying to match ObjectName '%s'... (length %zd)",TblFileDef.ObjectName,strlen(TblFileDef.ObjectName));
    while (i<NumSymbols)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            if (get_st_size(SymbolPtrs[UserObjSymbolIndex]) != 0)
            {
                ...
                {
                    printf("ELF file indicates object '%s' is of size %" PRId64 " but table definition structure indicates size %d",
