...
int32 GetTblDefInfo(void)
{
    ...
    if ((get_st_size(SymbolPtrs[TblDefSymbolIndex]) != sizeof(CFE_TBL_FileDef_t)) && (get_st_size(SymbolPtrs[TblDefSymbolIndex]) != 0))
    {
        printf("Error! '%s' is not properly defined in '%s'.  Size of object is incorrect (%ld).\n",
               TBL_DEF_SYMBOL_NAME, SrcFilename, (long int)get_st_size(SymbolPtrs[TblDefSymbolIndex]));
        Status = FAILED;
    }
    else
    {
...
int32 LocateAndReadUserObject(void)
{
    ...
    if (Verbose) printf("\nTrying to match ObjectName '%s'... (length %ld)",TblFileDef.ObjectName,(long int)strlen(TblFileDef.ObjectName));
    while (i<NumSymbols)
    {
        ...
        if (Verbose)
        {
            printf("strstr[%d] = %s; strlenSN = %ld; strlenON = %ld\n",i,strstr(SymbolNames[i], TblFileDef.ObjectName),
                                                                     (long int)strlen(SymbolNames[i]),
                                                                     (long int)strlen(TblFileDef.ObjectName));
        }
    ...
    if (Verbose)
    {
        ...
        if (i < NumSymbols)
        {
            ...
            printf("\nSymbolName length = %ld, ObjectName length = %ld\n",(long int)strlen(SymbolNames[i]), (long int)strlen(TblFileDef.ObjectName));
        }
    }
            ...
            if (get_st_size(SymbolPtrs[UserObjSymbolIndex]) != 0)
            {
                ...
                if (get_st_size(SymbolPtrs[UserObjSymbolIndex]) != TblFileDef.ObjectSize)
                {
                    printf("ELF file indicates object '%s' is of size %ld but table definition structure indicates size %d",
                           TblFileDef.ObjectName, (long int)get_st_size(SymbolPtrs[UserObjSymbolIndex]), TblFileDef.ObjectSize);
                    ...
                    printf("Size of %ld is assumed.\n", (long int)get_st_size(SymbolPtrs[UserObjSymbolIndex]));
                }
            }
            else
            {
